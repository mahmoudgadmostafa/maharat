...
const StudentProgressTable = ({ students = [], lessons = [], studentProgress = {} }) => {
  ...
  const calculateDetailedProgress = () => {
    return students.map(student => {
      // studentProgress هنا هو الكائن الذي يحتوي على تقدم جميع الطلاب، وليس تقدم طالب واحد
      // يجب أن نستخدم student.id للوصول إلى تقدم الطالب المحدد
      ...
      // Get all lesson IDs from the lessons prop
      ...
      // حساب إحصائيات الفيديو - عدد الدروس التي تم إكمال فيديوهاتها
      // هنا يجب أن نستخدم completedLessons من الـ progress object وليس من الـ events
      const videoCompletedLessonsCount = Object.keys(progress).filter(lessonId => 
      ...
      // حساب إحصائيات PDF - عدد الدروس التي تم فتح PDF فيها
      const pdfOpenedLessonsCount = Object.keys(progress).filter(lessonId => 
        progress[lessonId]?.pdfOpened
      ).length;
      const pdfOpenRate = totalLessonsCount > 0 ? (pdfOpenedLessonsCount / totalLessonsCount) * 100 : 0;
      
      // حساب إحصائيات الأسئلة - عدد الدروس التي تم الوصول لأسئلتها
      const questionsAccessedLessonsCount = Object.keys(progress).filter(lessonId => 
