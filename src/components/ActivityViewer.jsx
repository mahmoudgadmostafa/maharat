...
import { Target, ExternalLink, Eye, EyeOff, CheckSquare, CheckCircle } from 'lucide-react';
...
const ActivityViewer = ({ activityUrl, lessonId, index = 0, isVisible = true }) => {
  if (!isVisible) {
    return null;
  }
  ...
  const saveProgress = useCallback(async (unlocked, completed) => {
    ...
    try {
      const progressRef = doc(db, 'contentProgress', `${studentId}_${lessonId}_activity${index > 0 ? `_${index}` : ''}`);
  ...
  if (!activityUrl) {
    return <p className="text-gray-500">لا يوجد نشاط تعليمي لهذا الدرس.</p>;
  }

  const processedUrl = processGoogleDriveUrl(activityUrl);
      ...
      <CardHeader className="pb-3">
        <CardTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base sm:text-lg">النشاط التعليمي</span>
          </div>
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            ...
            <Button
              ...
              className={`${isCompleted ? 'bg-green-600 hover:bg-green-700' : 'bg-emerald-600 hover:bg-emerald-700'} text-white shadow-sm transition-all duration-300 disabled:opacity-50 disabled:grayscale min-h-[36px] flex-1 sm:flex-none`}
            >
              {isCompleted ? (
                <>
                  <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 ml-1" />
                  <span className="text-xs sm:text-sm">أنهيت النشاط ✓</span>
                </>
              ) : (
                <>
      ...
      {!isExpanded && (
        <CardContent className="pt-0">
          <p className="text-sm text-muted-foreground text-center">
            انقر على "عرض" لمشاهدة النشاط التعليمي في نفس الصفحة
          </p>
        </CardContent>
      )}
    </Card>
